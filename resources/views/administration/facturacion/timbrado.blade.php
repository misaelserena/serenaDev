...
<div id="container-cambio" class="div-search">
	<form action="">
		<center>
			<strong>BUSCAR CFDIs TIMBRADOS</strong>
		</center>
		<div class="divisor">
					...
					<tr>
						...
						<td class="align-middle">
							@if(\Storage::disk('reserved')->exists('/stamped/'.$bill->uuid.'.xml'))
								<a href="{{route('stamped.download.xml',$bill->uuid)}}" class="btn btn-blue" alt="XML" title="XML"><span class="icon-xml"></span></a>
							@endif
						</td>
						<td class="align-middle">
							@if(\Storage::disk('reserved')->exists('/stamped/'.$bill->uuid.'.pdf'))
								<a href="{{route('stamped.download.pdf',$bill->uuid)}}" class="btn btn-blue" alt="PDF" title="PDF"><span class="icon-pdf"></span></a>
							@endif
						</td>
						<td class="align-middle"><a href="{{route('bill.stamped.view',$bill->idBill)}}" class="btn btn-green"><span class="icon-search"></span></a></td>
					</tr>
				@endforeach
			</tbody>
		</table>
	</div>
	{{ $pending->appends(['folio'=> $folio, 'kind' => $kind, 'mindate' => $mindate,'maxdate' => $maxdate,'enterpriseid'=>$enterpriseid])->render() }}
@else
